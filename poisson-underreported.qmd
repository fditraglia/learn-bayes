...
\end{align*}
$$
so *ratios* of parameters are point-identified in this simple discrete example. The parameters themselves, however, are not. In particular, we can *never rule out* the possibility that 

**It appears that this example is *not* point identified, although we do learn the ratio of detection probabilities along with the ratio of means, and there is a testable restriction. To show lack of point identification, suppose WLOG that $\pi^H\geq \pi^L$. It's easy to show that so long as the testable restriction holds, we can never rule out $\pi^H=1$.**
