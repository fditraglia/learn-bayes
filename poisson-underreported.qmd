...
$$
The parameters themselves, however, are not identified. In particular, we can *never rule out* the possibility that $\pi^H=1$, i.e. there is no under-reporting.
To see why, suppose WLOG that $\pi^G \geq \pi^L$ and that the testable implication holds.
