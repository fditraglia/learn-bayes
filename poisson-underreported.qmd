...
Let's now consider the model under the additional complication that some of the thinned outcomes are censored. For a separate introduction to this setting, see e.g. `censored-data-STAN.qmd`.
...
As described in the notebook on censoring, we can write down the likelihood contribution for censored observations by integrating over the censored region of the thinned poisson process with rate $\mu_i \pi_i$ for all observations $i$ that have a censored outcome:
