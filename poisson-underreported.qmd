...
$$
The parameters themselves, however, are not identified. In particular, we can *never rule out* the possibility that there is no under-reporting for one of the two values that $Z$ can take on.
To see why, suppose WLOG that $m_0^\text{Hi} > m_0^\text{Lo}$ so that $\pi^L < \pi^H$. Suppose further that the model is correct, in which case its testable implication holds.
Then if $\pi^H = 1$ we have
$$
\begin{align*}
