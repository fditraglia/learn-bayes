...
Let's now consider the model under the additional compliction that some of the thinned outcomes are censored. For a separate introduction to this setting, e.g. `censored-data-STAN.qmd`. In particular, we assume that we observe the count $Y_i$ only if it is non-zero or above some **known** threshold $l$. $$
Y_i^{obs} = 
\begin{cases}
...
As described in the notebook on censoring, we can write down the likelihood contribution for censored observations as follows:
...
This is far from the true parameters. For the intercepts this is not to big of a surprise, but something is going wrong about finding β and δ... TODO!
