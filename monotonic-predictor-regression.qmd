...
get_survival_function <- function(land_dist) {
  nbins <- nrow(land_dist)
  c(1, (1 - cumsum(land_dist$frac_families))[-nbins])
}

land_distributions |> 
  compact(\(x) any(!is.na(x$frac_families)))

survival_functions <- land_distributions |>  
  map(get_survival_function) 
