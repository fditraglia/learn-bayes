...
- Parameters of Beta distribution are hard to interpret on their own.
- Instead of putting priors *directly* on $\alpha$ and $\beta$, put priors on
$$
\phi = \frac{\alpha}{\alpha + \beta}, \quad \kappa = \alpha + \beta, \quad
$$
- Inverse Transformation: $\alpha = \kappa \phi, \quad \beta = \kappa(1 - \phi)$
...
## Computing
- Rather than programming everything from scratch, use a **Probabilistic Programming Language** (PPL)
- I'll use STAN; other options include PyMC3 and Turing
- Specify model *structure* at a high level; rely on PPL to translate this into efficient code for drawing from the posterior 
- We'll have a future session on this.


## Visualizing the Hyper-prior
...
## Display the posterior mean for $\theta_j$
...
# Make a ggplot with MLE on the x-axis, mean on the y-axis, and confidence 
# "whiskers" constructed from q5 and q95: note the overplotting!
ggplot(Theta_results, aes(x = MLE, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = q5, ymax = q95), width = 0.001) +
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  labs(x = 'MLE', y = 'Posterior Mean', title = 'Raw vs Adjusted Estimates') +
  theme_minimal()

# Add jitter to the data
set.seed(123)  # for reproducibility
Theta_results_jittered <- Theta_results |> 
  mutate(MLE_jittered = MLE + runif(n(), -0.002, 0.002))

# Now plot with the jittered x-values
ggplot(Theta_results_jittered, aes(x = MLE_jittered, y = mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = q5, ymax = q95), width = 0.001) +
  geom_abline(intercept = 0, slope = 1, linetype = 'dashed') +
  labs(x = 'MLE', y = 'Posterior Mean', title = 'Raw vs Adjusted Estimates') +
...
## Display the posterior for $(\alpha, \beta)$


## Simulation showing how to get posterior inference for $\theta_{71}$ "by hand"
 
1. Draw a sample from the posterior for alpha, beta
2. For each draw, draw a sample from the posterior for theta_71 using the *exact* formula: theta_71 ~ Beta(y_71 + alpha, N_71 - y_71 + beta)
3. Plot the posterior for theta_71 and compare to the result from STAN as well as the crude approximation

```{r}
# Extract posterior draws for alpha and beta from STAN output
library(posterior)
alpha_beta_draws <- HB_beta_binomial_fit$draws(c('a', 'b')) |> 
  as_draws_df()

y_71 <- rats$y[71]
N_71 <- rats$N[71]

theta_71_sims_HB <- alpha_beta_draws |> 
...
theta_71_sims_HB |>
  ...
  labs(title = 'Posterior for Theta_71', x = expression(theta[71]), y = 'Density') +
  theme_minimal() + 
  geom_line(data = theta_71_crude, aes(x = x, y = y), color = 'red', linetype = 'dashed') 
```



## Exercises / Exhibits to add

- Make a picture showing the "raw" and "adjusted" estimates from each experiment
- Use simulation to explain the re-parameterization of the hyperprior
- How does the posterior for $\alpha, \beta$ compare to our "crude" approximation?
- Show how to use the posterior for $(\alpha, \beta)$ to get inference for $\theta_{71}$ "by hand" as in Gelman's example.
- Show how to generate predictions for a *new experiment* using the posterior for $(\alpha, \beta)$
...
- The choice of priors here is a bit tricky. See this [example](https://mc-stan.org/users/documentation/case-studies/pool-binary-trials.html). 
- To match the example in the book we could set $\alpha = \kappa \phi$ and $\beta = \kappa(1 - \phi)$ and them place a uniform distribution on $\phi$ and a Pareto(1, 1.5) distribution on $\kappa$. 
- But there's a potential issue here with centered versus non-centered parameterizations in HMC. So an alternative is to work on the logit scale and then use normal priors, in which case we can use the standard trick with non-centered parameterizations.
