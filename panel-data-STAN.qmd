...
mundlak1_fit$summary(variables = c('abar', 'tau', 'beta', 
                                   'gamma', 'delta', 'sigma')) |> 
  knitr::kable(digits = 2)
```
...
\mu_{it} &= \alpha_i + \beta X_{it} + \gamma Z_i + \delta U_i\\
\alpha_i &\sim \text{Normal}(\bar{\alpha}, \tau)\\
\bar{\alpha} &\sim \text{Normal}(0, 1)\\
\tau &\sim \text{Exponential}(1)\\
\beta, \gamma, \delta &\sim \text{Normal}(0, 1) \\ \\
...
\end{align*}
$$
The diagnostics don't look as good here, potentially because there are so many more parameters? Maybe there's a better way to implement it or maybe I need to think more carefulyl about priors?  I'm not sure. The results don't change much though:
```{r}
...
mundlak2_fit$summary(variables = c('abar', 'tau', 'beta', 
                                   'gamma', 'delta', 'sigma', 
                                   'kappa', 'lambda')) |>  
  knitr::kable(digits = 2)
```
